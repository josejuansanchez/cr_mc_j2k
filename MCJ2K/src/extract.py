...
# extract.py
#
# Extrae un número de capas de calidad de un vídeo codificado con más
# de una capa de calidad. El resultado es el mismo que haber
# especificado menos capas de calidad (a través de sus
# correspondientes "slopes") en el comando "compress".
#
# Secuencia típica de comandos:
#
...
# La opción de extraer las imágenes, transcodificarlas
# independientemente y volverlas a unir parece no funcionar. Probar
# opción parsear stream .mj2 directamente, dejando de copiar a la
# salida aquellos datos que pertenecen a capas de calidad que no deben
# ser extraídas. Además, esta opción es más eficiente y puede ser
...
while subband < temporal_levels:
    ...
    # Comanzamos la extracción de bytes:
    byte = f.read(1)
    while byte != '':
        if byte == '\xff':
            byte = f.read(1)
            if byte == '\xd9':
                ...
                # Nos saltamos los 4 últimos bytes de relleno.
