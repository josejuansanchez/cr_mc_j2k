...
#define TC_CPU_TYPE short		// <--- ORIGINAL 
//#define TC_CPU_TYPE unsigned char	// <--- MIO
...
(MVC_CPU_TYPE ****mv,           /* [PREV|NEXT][y_field|x_field][y_coor][x_coor] */
 ...
 class dwt2d < MVC_CPU_TYPE, MOTION_INTERPOLATION_FILTER < MVC_CPU_TYPE > > *mv_dwt) {
  ...
  /* PRUEBA */
  pic_dwt->synthesize(ref[PREV], pixels_in_y<<subpixel_accuracy, pixels_in_x<<subpixel_accuracy, subpixel_accuracy);
  pic_dwt->synthesize(ref[NEXT], pixels_in_y<<subpixel_accuracy, pixels_in_x<<subpixel_accuracy, subpixel_accuracy);
  ...
  */
  ...
  /* PRUEBA */  
  blocks_in_y = (pixels_in_y<<subpixel_accuracy)/block_size;
  blocks_in_x = (pixels_in_x<<subpixel_accuracy)/block_size;
  /**********/
...
int main(int argc, char *argv[]) {
  ...
  while(1) {
    ...
    case '?':
      ...
      printf("  Parameters:\n");
      printf("\n");
      printf("   -[-]block_size[v] = block size (%d)\n", block_size);
  ...
  pgmRead (even_fn, &rows_even, &cols_even, image);
  ...
  */
  ...
  for(int i=0; i<pictures/2; i++) {
    ...
    pgmRead (odd_fn, &rows_odd, &cols_odd, image);
