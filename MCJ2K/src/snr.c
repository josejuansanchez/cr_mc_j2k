...
int main (int argc, char *argv[]) {
  ...
  if (!strcmp(type,"uchar")) {
    unsigned char *buf_A = (unsigned char *)malloc(block_size);
    unsigned char *buf_B = (unsigned char *)malloc(block_size);
    int block;
  ...
  if (!strcmp(type,"ushort")) {
    ...
    for(;;) {
      ...
      unsigned short a,b;
      int ab;
      int r_A = fread(&a,sizeof(unsigned short),1,file_A);
      if(feof(file_A)) break;
      int r_B = fread(&b,sizeof(unsigned short),1,file_B);
      ...
      error=ab*ab;
      energy_error += error;
      //printf("%f %f %f\n",energy_A,energy_B,energy_error);
      //if(energy_A<=0) printf("error\n");
      //if(energy_B<=0) printf("error\n");
      //if(!(counter%1000)) printf("%Ld\n",energy_error);
      counter++;
    }
  }
