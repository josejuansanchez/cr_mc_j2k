...
int main(int argc, char *argv[])
{
  ...
  for(int y=0; y<blocks_in_y; y++) {
	...
	mv_C[prev_or_next][Y_FIELD][y][x] = mv_A[prev_or_next][Y_FIELD][y][x] + mv_B[prev_or_next][Y_FIELD][y][x];
	mv_C[prev_or_next][X_FIELD][y][x] = mv_A[prev_or_next][X_FIELD][y][x] + mv_B[prev_or_next][X_FIELD][y][x];
      }
  }
  ...
  sprintf(filename_out, "vectors/sum.dat");
