...
int main(int argc, char *argv[]) {
  ...
  while(1) {
    ...
    case '?':
      ...
      printf("   -[-r]eference_fn = input file with the reference pictures (\"%s\")\n", reference_fn);
  ...
  FILE *predicted_fd; {
    predicted_fd = fopen(predicted_fn,
#if defined ANALYZE
		    "r"
#else
		    "w"
  ...
  FILE *prediction_fd; char prediction_fn[80]; {
    sprintf(prediction_fn, "prediction_%s", predicted_fn);
  ...
  *image_dwt = new class dwt2d <
  ...
  int picture_border_size = search_range + block_overlaping;
#if defined DEBUG
  ...
  for(int c=0; c<COMPONENTS; c++) {
    image.read(reference_fd, reference[0][c], pixels_in_y[c], pixels_in_x[c]);
  }
  ...
  for(int i=0; i<pictures/2; i++) {
    ...
    /* Leemos siguiente la imagen (que es la que vamos a predicir). */

    for(int c=0; c<COMPONENTS; c++) {
      image.read(predicted_fd, predicted[c], pixels_in_y[c], pixels_in_x[c]);
    }
...
#if defined DEBUG
    info("%s: reading reference picture %d of \"%s\".\n",
    ...
    /* Luma. */
    for(int c=0; c<COMPONENTS; c++) {
      image.read(reference_fd, reference[1][c], pixels_in_y[c], pixels_in_x[c]);
    }
	...
	image.write(high_fd, residue[c], pixels_in_y[c], pixels_in_x[c]);
      ...
      /* No hay campo de movimiento (distinto de 0) asociado a una
	 imagen I. */
      ...
      /* Convertimos al rango [0,255] posiblemente con clipping y
	 escribimos a disco. */
...
#if defined DEBUG
    info("%s: writing predicted picture %d of \"%s\".\n",
	 argv[0], i, predicted_fn);
#endif
    ...
    for(int c=0; c<COMPONENTS; c++) {
      image.write(predicted_fd, predicted[c], pixels_in_y[c], pixels_in_x[c]);
    }
