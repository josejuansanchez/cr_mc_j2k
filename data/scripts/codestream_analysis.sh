...
while [ $i -le $CLAYERS ]; do
  BYTES=`kdu_transcode -i $IMAGES_J2C_DESTINATION/$first_image.j2c -o $IMAGES_J2C_DESTINATION/${i}.j2c Clayers=${i} | grep written | awk '{ print $5}'`
  echo -e "Layer $i     Size: $BYTES bytes"
  i=$(($i+1))
done
