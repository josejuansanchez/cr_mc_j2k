...
static kdl_message_info data[301] = {
  {
  ...
  },
  {
    ...
    " (starting from 0) in the multi-component transform for some tile (or in the main header).  Either the `Mstages' attribute contains insufficient entries, or no `Mstage_blocks' attribute exists with the instance identifier provided by `Mstages'.\0"
  },
  {
    "E(codestream.cpp)", 134743299,
    "Kakadu Core Error:\n",
    "Multi-component transform blocks must each have a strictly positive number of inputs and outputs, as identified by the `Mstage_blocks' parameter attribute.\0"
  },
  {
  ...
  },
  {
    ...
    "Kakadu Core Error:\n",
    "Except for irreversible matrix-based decorrelation transforms, all multi-component transform blocks described by the `Mstage_blocks' and `Mstage_xforms' parameter attributes must produce exactly one output component for each of their input components.\0"
  },
  {
  ...
  },
  {
    ...
    "Kakadu Core Error:\n",
    "Malformed `Mstage_blocks' attribute encountered in `mcc_params::finalize'.  Each transform block must be assigned a strictly positive number of input and output components.\0"
  },
  {
    "E(params.cpp)", 554108166,
    "Kakadu Core Error:\n",
    "Malformed `Mstage_blocks' attribute encountered in `mcc_params::finalize'.  The transform blocks must together consume all input components defined by `Mstage_inputs' (no more and no less) and produce all output components defined by `Mstage_outputs' (no more and no less).\0"
  },
  {
  ...
  },
  {
    ...
    "Kakadu Core Error:\n",
    "Malformed `Mstage_xforms' attribute encountered in `mcc_params::finalize'.  The number of records in this attribute must be identical to the number of records in `Mstage_blocks'.\0"
  },
  {
...
static bool dummy_val = kdl_register_text(data,301);

#ifndef KDU_EXCLUDE_DEVELOPER_TEXT

static kdl_message_info dev_data[129] = {
  {
    "E(codestream.cpp)", 0,
    "Kakadu Core Error:\n",
  ...
  },
  {
    "E(codestream.cpp)", 59,
    "Kakadu Core Error:\n",
    "When generating code-stream output incrementally, each call to `kdu_codestream::flush' must provide the same number of quality layer specifications.\0"
  },
  {
  ...
  {
    "E(codestream.cpp)", 302450432,
    "Kakadu Core Error:\n",
    "You have invoked `kdu_codestream::destroy' from one thread, thread queues remain in which other threads may potentially do work.  This is OK, so long as the thread from which you invoke `kdu_codestream::destroy' is one of the threads participating in the `kdu_thread_entity' group to which the above-mentioned thread queues belong.  However, this does not appear to be the case here.  Typically, it is best to destroy codestreams from the same thread which invokes the `kdu_thread_entity::create' and `kdu_thread_entity::add_thread' functions which build cooperating multi-threaded processing systems.\0"
  },
  {
    "E(compressed.cpp)", 5,
    "Kakadu Core Error:\n",
  ...
  },
  {
    "W(codestream.cpp)", 6,
    "Kakadu Core Warning:\n",
    "The compressed data buffer server is being destroyed before all allocated buffers have been returned.  The problem is most likely connected with a bug in the code-block destruction code.\0"
  },
  {
    "W(codestream.cpp)", 7,
    "Kakadu Core Warning:\n",
    "The compressed data buffer server is being destroyed before all size accounting associated with fixed data structures has been completed.  This is most likely due to a bug in the memory accounting logic used for dynamic cache management.\0"
  },
  {
    "W(codestream.cpp)", 15,
    "Kakadu Core Warning:\n",
...
static bool dev_dummy_val = kdl_register_text(dev_data,129);
