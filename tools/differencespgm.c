...
int main (int argc, char *argv[])
{
    ...
    if (argc!=7)
    {
	   printf("\nUso: %s <in_filenameA> <in_filenameB> <precision> <precinct_size> <out_filename_precincts_list> <metric_distance>.",argv[0]);
    ...
    if (rowsA==rowsB && colsA==colsB)
    {
	   ...
	   averageStats(precincts,np,avg,&contUp,&contDown,&contEqual);
	   printf("\n\nAverage: %lf",avg);
	   printf("\nContUp: %ld. ContDown: %ld. ContEqual: %ld.\n",contUp,contDown,contEqual);

	   /* Guardamos la lista de precintos en disco */
	   writeOK = writePrecinctsToFile(precincts,np,argv[5]);
	   if (writeOK)
	   {
       ...
       /* Inicializamos la matriz de la imagen de salida con el valor 255 */
       initializeImage(imageD,rowsA,colsA,255);

	   /* Generamos una imagen con los precintos que están por encima de la media */
	   precinctsAvgUpToImage(precincts,np,avg,imageB,imageD,rowsA,colsA,precinctSize);
	   
       /* Guardamos la imagen en disco */
       writeOK = pgmWrite (FILENAME_AVG_UP,rowsA,colsA,imageD,"Imagen precintos");
       if (writeOK) 
	   {
	       printf("\nEl archivo: %s, con la imagen de precintos UP se ha creado con éxito.",FILENAME_AVG_UP);
	   }
	   else	
	   {
	       printf("\nError al crear la imagen UP: %s.",FILENAME_AVG_UP);
       }

       /* Inicializamos la matriz de la imagen de salida con el valor 255 */
       initializeImage(imageD,rowsA,colsA,255);

	   /* Generamos una imagen con los precintos que están por debajo de la media */
	   precinctsAvgDownToImage(precincts,np,avg,imageB,imageD,rowsA,colsA,precinctSize);
	
       /* Guardamos la imagen en disco */
       writeOK = pgmWrite (FILENAME_AVG_DOWN,rowsA,colsA,imageD,NULL);
       if (writeOK) 
	   {
	       printf("\nEl archivo: %s, con la imagen de precintos DOWN se ha creado con éxito.",FILENAME_AVG_DOWN);
	   }
	   else	
	   {
	       printf("\nError al crear la imagen DOWN: %s.",FILENAME_AVG_DOWN);
        }

	   /* Guardamos la imagen residuo en disco */
       writeOK = pgmWrite (FILENAME_RESIDUO,rowsA,colsA,imageC,NULL);
       if (writeOK) 
	   {
