/*
 * These routines read PPM bitmaps (types P2 and P5)
 * and write out PPM files in binary (P5) format.
 * Note that lines in PPM files should be no longer than 70
 * characters long.
 *
 * PPM files have a maximum value of 255 for each pixel (8 bit greyscale)
 *
 * NOTE:
...
int ppmRead (char *fileName,long *rows,long *cols,unsigned char **image) 
{
      ...
      *  maximum value is illegal.*/
      ...
      if ((*cols)<1 ||(*rows)<1 || maximumValue<0 || maximumValue>MAXVALUE){
        ...
        printf ("ERROR: increase MAXROWS/MAXCOLS in PPM.h");
        fclose (filePointer);
        return (0);
      }

      /* Read in the data for binary (P6) or ascii (P2) PPM formats   */
      if (binary) {
        for (i = 0; i < (*rows); i++) {
          numberRead += fread((void *)&(image[i][0]),sizeof(unsigned char),(*cols)*3,filePointer);
 ...
 *   (in P5 PPM format (binary)).  A 1 is returned if the write was completed
...
int ppmWrite(char* filename, long rows,long cols,unsigned char **image,char* comment_string) 
{
      ...
      for (i=0; i < rows; i++) {
          nwritten += fwrite((void*)&(image[i][0]),sizeof(unsigned char), cols*3, file);
      }
