...
int main (int argc, char *argv[])
{
    ...
    if (argc!=7)
    {
        printf("\nUso: %s <in_pgm_filenameA> <in_pgm_filenameB> <filename_precincts_list> <precinct_size> <out_filename> <max_precincts>.",argv[0]);
        printf("\n\nin_filenameA = Image PGM. (Background Image).");
        printf("\nin_filenameB = Image PGM. (All Quality Layers. Next Image).");
    ...
    /* Validamos el valor del tamaño de precinto */
    maxPrecincts = atoi(argv[6]);
    if (maxPrecincts<0 || precinctSize>(MAXROWS/precinctSize)*(MAXCOLS/precinctSize))
    {
        printf("\nEl valor de maxPrecincts es incorrecto.\n");
        exit(0);
    }
    ...
    if (rowsA==rowsB && colsA==colsB)
    {
        ...
        /* Generamos una imagen con los precintos con los primeros maxPrecincts */
        MaxPrecinctsToImage(precincts,np,maxPrecincts,imageB,imageA,rowsA,colsA,precinctSize);
